defmodule CrawlyUIWeb.JobControllerTest do
  ...
  @create_attrs %{spider: "some spider", state: "some state"}
  @update_attrs %{spider: "some updated spider", state: "some updated state"}
  @invalid_attrs %{spider: nil, state: nil}

  def fixture(:job) do
    {:ok, job} = Manager.create_job(@create_attrs)
    job
  end
  ...
  describe "update job" do
    ...
    test "redirects when data is valid", %{conn: conn, job: job} do
      ...
      assert html_response(conn, 200) =~ "some updated spider"
    end
